...
let activeUnsupportedFlag: string;
...
beforeEach(() => {
  ...
  activeUnsupportedFlag = join(binDir, "gh-active-unsupported");
...
case "$1 $2" in
  ...
  "auth status")
    ...
    if [ -e "${activeUnsupportedFlag}" ]; then
      case " $* " in *" --active "*) echo "unknown flag: --active" >&2; exit 1;; esac
    fi
...
describe("github plugin gh auth probe (#1758)", () => {
  ...
  it("falls back for gh versions that reject the active-account flag", async () => {
    writeFileSync(activeUnsupportedFlag, "");
    const { harness } = await loadWithSyncServiceOnce();
    expect(harness.needsConfigurationMessages).toEqual([]);
    expect((await harness.callRpc("status")) as { ghState: string }).toMatchObject({
      ghState: "ready",
    });
    expect(ghCalls()).toContain("auth status --hostname github.com --active");
    expect(ghCalls()).toContain("auth status --hostname github.com");
  });
